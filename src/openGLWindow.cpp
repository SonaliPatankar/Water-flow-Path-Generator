...
void OpenGLWindow::setData(const std::vector<Geometry::Point3D>& inTerrainVertices,
    const std::vector<Geometry::Point3D>& inWaterPathVertices)
{
    mTerrainVertices.clear();
    mTerrainColors.clear();
    mWaterPathVertices.clear();
    mWaterPathColors.clear();
    ...
    for (const auto& point : inWaterPathVertices) 
    {
        mWaterPathVertices.push_back((point.x()));
        mWaterPathVertices.push_back((point.y()));
        mWaterPathVertices.push_back((point.z()));

        // blue color
        mWaterPathColors.push_back(0.0f); 
        mWaterPathColors.push_back(0.0f); 
        mWaterPathColors.push_back(1.0f); 
    }
...
void OpenGLWindow::paintGL()
{
    ...
    glLineWidth(5.0f);
    glVertexAttribPointer(m_posAttr, 3, GL_DOUBLE, GL_FALSE, 0, mWaterPathVertices.data());
    glVertexAttribPointer(m_colAttr, 3, GL_DOUBLE, GL_FALSE, 0, mWaterPathColors.data());
    glDrawArrays(GL_LINES, 0, mWaterPathVertices.size() / 3);
